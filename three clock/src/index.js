...
(async function init() {
  ...
  ring1 = CustomRing(envRT, 0.65, "red");
  ring1.scale.set(0.75, 0.75);
  scene.add(ring1);
  
  ring2 = CustomRing(envRT, 0.35,"blue");
  ring2.scale.set(1.05, 1.05);
  scene.add(ring2);

  ring3 = CustomRing(envRT, 0.15, "green");
  ring3.scale.set(1.3, 1.3);
  scene.add(ring3);

  let hourLine = Line(0.4, 0.135, 0.07, envRT, "red", 3);
  scene.add(hourLine);

  let minutesLine = Line(0.8, 0.135, 0.07, envRT, "green", 1);
  scene.add(minutesLine);

  let secondsLine = Line(1, 0.075, 0.07, envRT, "blue", 1);
  ...
  renderer.setAnimationLoop(() => {
    ring1.rotation.x = ring1.rotation.x * 1.0 + mousePos.y * 0.05 * 0.4;
    ring1.rotation.y = ring1.rotation.y * 1.0 + mousePos.x * 0.05 * 0.4;

    ring2.rotation.x = ring2.rotation.x * 0.9 + mousePos.y * 0.05 * 0.375;
    ring2.rotation.y = ring2.rotation.y * 0.9 + mousePos.x * 0.05 * 0.375;
